...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <form method="GET" action="{{ route('panel.productos') }}" class="d-flex align-items-center gap-3"
                id="searchForm">
                <div class="input-group" style="max-width: 300px;">
                    <input type="text" name="search" id="searchInput" class="form-control"
                        placeholder="Buscar productos..." value="{{ $search ?? '' }}" autocomplete="off">
                                    ...
                                    <td class="text-center align-middle">
                                        <div class="form-check form-switch d-inline-block">
                                            <input type="checkbox" class="form-check-input visibility-toggle"
                                                data-id="{{ $producto->id }}" {{ $producto->visible ? 'checked' : '' }}
                    ...
                    fetch(`/panel/productos/${productId}/visibility`, {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                badge.className =
                                    `badge bg-${data.visible ? 'success' : 'danger'} ms-2`;
