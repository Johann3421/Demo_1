...
$seoTemplates = [
    'Laptops' => [
        ...
        'description' => 'Laptop {nombre} {marca} con {procesador} y {ram} RAM. Ideal para {uso}. ¡Compra ahora con garantía!',
        'keywords' => '{nombre}, {marca}, laptop {modelo}, laptop {procesador}, laptop {ram}, laptop {uso}, comprar laptop',
    ],
    'Componentes para pc' => [
        'title' => '{nombre} {marca} | Mejora tu PC - SEKAITECH',
        'description' => '{nombre} {marca}: {descripcion}. ¡Optimiza tu rendimiento con nuestros componentes de PC!',
        'keywords' => '{nombre}, {marca}, componente pc {modelo}, {tipo_componente}, mejorar pc, comprar componentes pc',
    ],
    'Monitores' => [
        'title' => '{nombre} {marca} | Alta Resolución - SEKAITECH',
        'description' => 'Monitor {nombre} {marca} con {resolucion} y {tasa_refresco}. ¡Imágenes nítidas para trabajar y jugar!',
        'keywords' => '{nombre}, {marca}, monitor {modelo}, monitor {resolucion}, monitor {tasa_refresco}, comprar monitor',
    ],
    'Impresoras' => [
    ...
    'Computadoras' => [
        ...
        'description' => 'Computadora {nombre} {marca} con {procesador} y {ram} RAM. Ideal para {uso}. ¡Compra ahora con garantía!',
        'keywords' => '{nombre}, {marca}, computadora {modelo}, computadora {procesador}, computadora {ram}, computadora {uso}, comprar computadora',
    ],
];
...
$title = str_replace(
    ['{nombre}', '{marca}', '{modelo}', '{procesador}', '{ram}', '{uso}', '{resolucion}', '{tasa_refresco}', '{descripcion}', '{tipo_componente}', '{tipo_impresora}'],
    [ucwords($producto->nombre), ucwords($producto->marca), $producto->modelo, $producto->procesador, $producto->ram, $producto->uso, $producto->resolucion, $producto->tasa_refresco, $producto->descripcion, $producto->tipo_componente, $producto->tipo_impresora],
    $seo['title']
);

$description = str_replace(
    ['{nombre}', '{marca}', '{modelo}', '{procesador}', '{ram}', '{uso}', '{resolucion}', '{tasa_refresco}', '{descripcion}', '{tipo_componente}', '{tipo_impresora}'],
    [$producto->nombre, $producto->marca, $producto->modelo, $producto->procesador, $producto->ram, $producto->uso, $producto->resolucion, $producto->tasa_refresco, $producto->descripcion, $producto->tipo_componente, $producto->tipo_impresora],
    $seo['description']
);
...
@section('meta-description', $description . ' Precio: $' . number_format($producto->precio_dolares, 2) . ' | S/' . number_format($producto->precio_soles, 2) . '. ¡Garantía y soporte técnico incluido! Compra ahora y aprovecha nuestras ofertas exclusivas.')

@section('keywords', $keywords . ', precio ' . number_format($producto->precio_dolares, 2) . ' dólares, precio ' . number_format($producto->precio_soles, 2) . ' soles, comprar ' . strtolower($producto->nombre))
    ...
    <div class="product-container">
        ...
        <div class="product-gallery">
            ...
            <div class="product-main-image">
                <img
                    src="{{ asset('images/' . $producto->imagen_url) }}"
                    alt="{{ $producto->nombre }}"
                ...
                <ul class="purchase-features">
                    <li class="purchase-feature">
                        <svg class="feature-icon" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M12,2L4,5V11.09C4,16.14 7.41,20.85 12,22C16.59,20.85 20,16.14 20,11.09V5L12,2M12,7C13.66,7 15,8.34 15,10C15,11.66 13.66,13 12,13C10.34,13 9,11.66 9,10C9,8.34 10.34,7 12,7M12,15.5C14.33,15.5 18,16.56 18,17.5V18.5H6V17.5C6,16.56 9.67,15.5 12,15.5Z" />
                        </svg>
                        <span class="feature-text">Pago con tarjeta <strong class="highlight">¡¡Sin recargos!!</strong></span>
                    </li>
                    <li class="purchase-feature">
                        <svg class="feature-icon" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M12,1L3,5V11C3,16.55 6.84,21.74 12,23C17.16,21.74 21,16.55 21,11V5L12,1M12,7C13.4,7 14.8,8.1 14.8,9.5V11C15.4,11 16,11.6 16,12.2V15.7C16,16.4 15.4,17 14.7,17H9.2C8.6,17 8,16.4 8,15.8V12.2C8,11.6 8.6,11 9.2,11V9.5C9.2,8.1 10.6,7 12,7M12,8.2C11.2,8.2 10.5,8.7 10.5,9.5V11H13.5V9.5C13.5,8.7 12.8,8.2 12,8.2Z" />
                        </svg>
                        <span class="feature-text">Precio incluye el <strong class="highlight">I.G.V</strong></span>
                    </li>
                    <li class="purchase-feature">
                        <svg class="feature-icon" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M12,2C6.48,2 2,6.48 2,12C2,17.52 6.48,22 12,22C17.52,22 22,17.52 22,12C22,6.48 17.52,2 12,2M12,20C7.59,20 4,16.41 4,12C4,7.59 7.59,4 12,4C16.41,4 20,7.59 20,12C20,16.41 16.41,20 12,20M13,17H11V15H13V17M13,13H11V7H13V13Z" />
                        </svg>
                        <span class="feature-text">Precio sujeto a cambios sin previo aviso</span>
                    </li>
                    <li class="purchase-feature">
                        <svg class="feature-icon" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M19,15H18A3,3 0 0,1 15,18V19A3,3 0 0,1 12,22A3,3 0 0,1 9,19V18A3,3 0 0,1 6,15H5A3,3 0 0,1 2,12A3,3 0 0,1 5,9H6A3,3 0 0,1 9,6V5A3,3 0 0,1 12,2A3,3 0 0,1 15,5V6A3,3 0 0,1 18,9H19A3,3 0 0,1 22,12A3,3 0 0,1 19,15M12,4A1,1 0 0,0 11,5V6A1,1 0 0,0 12,7A1,1 0 0,0 13,6V5A1,1 0 0,0 12,4M12,20A1,1 0 0,0 13,19V18A1,1 0 0,0 12,17A1,1 0 0,0 11,18V19A1,1 0 0,0 12,20M5,11A1,1 0 0,0 6,12A1,1 0 0,0 5,13A1,1 0 0,0 4,12A1,1 0 0,0 5,11M19,11A1,1 0 0,0 20,12A1,1 0 0,0 19,13A1,1 0 0,0 18,12A1,1 0 0,0 19,11Z" />
                        </svg>
